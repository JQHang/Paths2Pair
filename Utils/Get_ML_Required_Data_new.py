...
def get_ML_required_pandas_data(index,aim_UID_pd,sample_source_str, time_range_str, aim_ML_file_store_name, aim_node_type,
    ...
    for tmp_meta_path_file in tmp_meta_path_file_list:
        ...
        for tmp_feature_type in Feature_Type_list:
            ...
            # 修正列名(除了目标列，都加上后缀)
            tmp_node_feature_pd.columns = tmp_node_feature_pd.columns.map(lambda x: x + '^' + tmp_feature_type if x != last_column_name else x)
        ...
        # 修正列名
        tmp_related_UID_feature_pd.columns = tmp_related_UID_feature_pd.columns.map(lambda x: '^'.join(filter(None, x)))
        ...
        # 给剩余列的列名加上元路径名称
        tmp_result_column_feature_pd.columns = tmp_meta_path_name + '|' + tmp_result_column_feature_pd.columns
    ...
    if not os.path.exists(tmp_output_processed_aim_node_feature_dir +'_'+str(index)+ 'output.pkl'):
        ...
        for tmp_feature_type in Feature_Type_list:
            ...
            # 修正列名(除了目标列，都加上后缀)
            tmp_node_feature_pd.columns = tmp_node_feature_pd.columns.map(lambda x: x + '^' + tmp_feature_type if x != (aim_node_type + '_UID') else x)
        ...
        # 给剩余列的列名加上特征来源名称
        aim_node_feature_pd.columns = 'Aim_Node_Feature|' + aim_node_feature_pd.columns
