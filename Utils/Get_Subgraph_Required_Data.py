...
def get_subgraph_required_pandas_data(sample_source_str, time_range_str, aim_node_type, subgraph_hop_num, regenerate = False,
    ...
    if not regenerate and os.path.exists(tmp_all_output_data_time_range_subgraph_GNN_Feature_dir + 'Target_Node_with_Index.pkl'):
        ...
        # 添加是否是Target_node的信息
        aim_UID_pd['Target'] = True
        
        # 按新序号重新排列标签表中的标签序号
        aim_UID_with_new_index_pd = tmp_node_UID_to_index_dict[aim_node_type].merge(aim_UID_pd, on = aim_node_type + '_UID', 
...
#                                       how = 'left').reset_index(drop = True)
    ...
    # 只保留是Target_Node的行
    Target_Node_UID_with_new_index_pd = aim_UID_with_new_index_pd[aim_UID_with_new_index_pd['Target'] == True]
    
    # 保存目标节点的信息到返回信息中
    Processed_Subgraph_Data_dict['Target_Node_Index'] = Target_Node_UID_with_new_index_pd['index'].values
    ...
    if 'Label' in aim_UID_with_new_index_pd.columns:
        Processed_Subgraph_Data_dict['Target_Node_Label'] = Target_Node_UID_with_new_index_pd['Label'].values
        
        aim_UID_with_new_index_pd['Label'] = aim_UID_with_new_index_pd['Label'].fillna(-1)
        Processed_Subgraph_Data_dict['All_Target_Node_Label'] = aim_UID_with_new_index_pd['Label'].values
    #######################################################################################################################
    # 依次处理各跳的关系
