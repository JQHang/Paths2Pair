...
返回值：
    目标节点的特征值
"""
def Get_Proc_Feature_Data_From_Online(Spark_Session, Feature_Table_Config_list, regenerate, tmp_Aim_Feature_Table_dt_list, Aim_Node_type, 
    ...
    # 设置各类型特征的保存位置
    Aim_Node_Raw_Feature_file = (Feature_Data_From_Online_Time_Store_dir + Node_Comment + '_Raw.pkl')
    Aim_Node_Norm_Feature_file = (Feature_Data_From_Online_Time_Store_dir + Node_Comment + '_Norm.pkl')
    Aim_Node_Std_Feature_file = (Feature_Data_From_Online_Time_Store_dir + Node_Comment + '_Std.pkl')
    ...
    if regenerate or not os.path.exists(Aim_Node_Std_Feature_file):
        ...
        for tmp_feature_table_Info_dict in Feature_Table_Config_list:
            ...
            # 如果没有对应的总结表，则得保证都是特征列，并进行相关检测和总结表计算:
            if "Summary_Table_Name" not in tmp_feature_table_Info_dict:
                # 记录有效的特征列名
                ...
                print('有总结表:', tmp_feature_table_Info_dict["Summary_Table_Name"],
                    '跳过特征列格式检测,全部列数为:', len(tmp_feature_table_rdd_json))
                ...
                tmp_sql_command = """
                            SELECT
                                *
                            FROM
                                """ + tmp_feature_table_Info_dict["Summary_Table_Name"] + """
                            WHERE 
                                dt = '""" + tmp_Aim_Feature_Table_dt_list[0] + """'"""
                ...
                tmp_feature_table_summary_rdd = Spark_Session.sql(tmp_sql_command)

                # 转化为pandas格式
                tmp_feature_table_summary_pd = tmp_feature_table_summary_rdd.toPandas()
...
#             print(tmp_aim_node_feature_pandas_raw.dtypes)
            ...
            tmp_aim_node_feature_pandas_raw.to_pickle('tmp1.pkl')
            tmp_feature_table_summary_pd.to_pickle('tmp2.pkl')
